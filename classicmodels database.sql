...
#1.	Fetch the employee number, first name and last name of those employees
...
select productcode,sum(quantityOrdered)total_ordered from orderdetails group by productcode order by  total_ordered desc
limit 10 ; 

#2 2.	Company wants to analyze payment frequency by month. Extract the month name from the payment date to count the total number of payments for each month and include only those months with a payment count exceeding 20 (Refer Payments table). 
...
#CONSTRAINTSCONSTRAINTS: Primary, key, foreign key, Unique, check, not null, default
...
Create table  Orders (
    ...
    constraint check_total_amt CHECK (total_amount >= 0)
);
...
gender char(9) ,check (gender in ('Male' or 'Female')),
Managerid int 
);
...
join project m on e.managerid = m.employee_id where
...
modify column Facilityid int auto_increment PRIMARY KEY;
...
# Checking Classic Cars
SELECT COUNT(*) AS ClassicCarsCount
FROM products
WHERE productLine = 'Classic Cars' AND buyPrice > (SELECT AVG(buyPrice) FROM products);

# Checking Vintage Cars
SELECT COUNT(*) AS VintageCarsCount
FROM products
WHERE productLine = 'Vintage Cars' AND buyPrice > (SELECT AVG(buyPrice) FROM products);
...
call InsertIntoEmp_EH(2, 'sri sudha', 'sri.sudha@example.com');
